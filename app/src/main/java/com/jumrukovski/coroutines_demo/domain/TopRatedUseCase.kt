...
class TopRatedUseCase @Inject constructor(private val repository: TvRepository, private val scope: CoroutineScope) :
    BaseUseCase<ApiResponse<List<Item>>>() {

    override fun getItems(language: String, page: Int): LiveData<ApiResponse<List<Item>>> {
        scope.launch {
            response.postValue(repository.getTopRatedTvShows(language, page))
        }
